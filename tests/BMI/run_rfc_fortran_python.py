...
def create_rfc_values():
    rfc_da_df = xr.open_dataset(
        rfc_timeseries_folder + "2021-10-20_00.60min.KNFC1.RFCTimeSeries.ncdf", engine="netcdf4"
    ).to_dataframe()
    return {
...
# --------------------------------------------------------
levelpool = MC_Levelpool(0, values["lake_number"], array("l", values["upstream_ids"]), args, int(1))
