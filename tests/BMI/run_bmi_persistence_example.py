...
full_model.set_value(
    "dx", np.array([3383.185374, 7247.358765, 2090.045820, 2059.280869, 3909.862245, 1568.900922])
)
...
full_model.set_value(
    "twcc", np.array([14.304953, 14.045269, 96.895086, 33.389247, 100.671812, 49.287396])
)
...
for hr in range(120):
    ...
    flowveldepth = pd.DataFrame(
        full_model.get_value("fvd_results").reshape(6, 36), index=full_model.get_value("fvd_index")
    )
    ...
    flowveldepth = pd.DataFrame(
        upper_routing_model.get_value("fvd_results").reshape(len(fvd_index), fvd_num_columns),
        index=fvd_index,
    )
    ...
    flowveldepth = pd.DataFrame(
        lower_routing_model.get_value("fvd_results").reshape(len(fvd_index), fvd_num_columns),
        index=fvd_index,
    )
