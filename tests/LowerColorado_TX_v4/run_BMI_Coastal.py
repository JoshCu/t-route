...
def run_troute(config):
    ...
    config_dict = {
        "Standard AnA": {"yaml": "/configs/Standard_Coastal_AnA.yaml", "duration": 3, "restart": 2},
    }
    ...
    troute.set_value(
        "stationStringLengthArray_usgs", DAforcing.get_value("stationStringLengthArray_usgs")
    )
    ...
    troute.set_value(
        "datesSecondsArray_reservoir_usgs", DAforcing.get_value("datesSecondsArray_reservoir_usgs")
    )
    troute.set_value("nDates_reservoir_usgs", DAforcing.get_value("nDates_reservoir_usgs"))
    troute.set_value(
        "stationArray_reservoir_usgs", DAforcing.get_value("stationArray_reservoir_usgs")
    )
    troute.set_value(
    ...
    troute.set_value(
        "stationArray_reservoir_usace", DAforcing.get_value("stationArray_reservoir_usace")
    )
    troute.set_value(
    ...
    troute.set_value(
        "rfc_StationId_stringLengths", DAforcing.get_value("rfc_StationId_stringLengths")
    )
    ...
    for hr in range(config_dict[config]["duration"]):
        ...
        # Set dynamic values
        troute.set_value(
            "land_surface_water_source__volume_flow_rate", np.array(forcing_vals.iloc[:, 0])
        )
