...
class Config(BaseModel):
    ...
    def check_levelpool_filepath(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if waterbody_parameters:
            ...
            if simulate_waterbodies and network_type == "NHDNetwork":
                assert levelpool, (
                    "Waterbody simulation is enabled for NHDNetwork, but level pool parameters are missing."
                )
                levelpool_file = levelpool.level_pool_waterbody_parameter_file_path
                assert levelpool_file, (
                    "Waterbody simulation is enabled for NHDNetwork, but no level pool parameter file is provided."
                )
    ...
    def check_diffusive_domain(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hybrid_parameters:
            ...
            if run_hybrid:
                assert hybrid_parameters.diffusive_domain, (
    ...
    def check_topobathy_domain(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hybrid_parameters:
            ...
            if use_natl_xsections:
                assert hybrid_parameters.topobathy_domain, (
                    "Use natural cross-sections is enabled, but topobathy domain file is missing."
                )
    ...
    def check_refactored(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hybrid_parameters:
            ...
            if run_refactored_network:
                assert hybrid_parameters.refactored_domain, (
                    "Run refactored network is enabled, but refactored domain file is missing."
                )
                assert hybrid_parameters.refactored_topobathy_domain, (
                    "Run refactored network is enabled, but refactored topobathy domain file is missing."
                )
    ...
    def check_coastal_domain(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hybrid_parameters:
            ...
            if coastal_boundary_input_file:
                assert hybrid_parameters.coastal_boundary_domain, (
                    "Coastal boundary forcing files provided, but coastal boundary domain file is missing."
                )
        ...
        if da_parameters:
            ...
            if streamflow_DA:
                ...
                if streamflow_nudging and network_type == "NHDNetwork":
                    assert streamflow_DA.gage_segID_crosswalk_file, (
                        "Streamflow nudging is enabled on NHDNetwork, but gage_segID_crosswalk_file is missing."
                    )
            ...
            if reservoir_da:
                ...
                if reservoir_rfc_forecasts:
                    ...
                    if not reservoir_parameter_file and network_type == "NHDNetwork":
                        error_message += (
                            " Reservoir_parameter_file is missing (and network type is NHDNetwork)."
                        )
                ...
                if reservoir_persistence_usace:
                    ...
                    if not usace_timeslices_folder:
                        ...
                        if not os.path.exists(usace_timeslices_folder):
                            error_message += (
                                " USACE_timeslices_folder path provided does not exist. "
                            )
    ...
    def check_wrf_hydro_restart_files(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if restart_parameters:
            ...
            if wrf_hydro_channel_restart_file:
                assert restart_parameters.wrf_hydro_channel_ID_crosswalk_file, (
                    "WRF-Hydro channel restart file provided, but wrf_hydro_channel_ID_crosswalk_file file is missing."
                )
    ...
    def check_start_datetime(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if restart_parameters:
            ...
            if not (wrf_hydro_channel_restart_file or lite_channel_restart_file):
                assert restart_parameters.start_datetime, (
                    "No start_datetime provided in config file for cold start (no restart files)."
                )
    ...
    def check_flowpath_edge_list(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if Path(geo_file_path).suffix == ".json":
            assert flowpath_edge_list, (
                "geo_file_path is json, but no flowpath_edge_list is provided."
            )
            assert Path(flowpath_edge_list).suffix == ".json", (
    ...
    def check_lite_restart_directory(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if values["output_parameters"]:
            ...
            if lite_restart is not None:
                ...
                assert lite_restart_directory, (
                    "lite_restart is present in output parameters, but no lite_restart_output_directory is provided."
                )
