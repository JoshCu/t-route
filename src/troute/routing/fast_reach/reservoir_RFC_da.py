...
def _search_RFCTimeSeries_files_backward_from_offset_hours(
    offset_date, max_rfc_timeseries_file_search_hours, rfc_gage_id, rfc_timeseries_folder
):
    """
...
def _timeseries_idx_updatetime_totalcounts(lookback_hours, rfc_da_df, rfc_timeseries_offset_hours):
    """
    Arguments
    ---------
    ...
    timeseries_idx = int(
        lookback_seconds / time_step_seconds + observed_counts - rfc_timeseries_offset_hours
    )
...
def _validate_RFC_data(
    ...
    if all(synthetic) == 1:
        use_RFC = False
        print(
            f"WARNING: RFC Forecast Time Series discharges for reservoir {lake_number} \
              are all synthetic. \
        ...
        use_RFC = False
        print(
            f"WARNING: RFC Forecast Time Series file for reservoir {lake_number} \
              does not exist. \
              This reservoir will use level pool calculations instead."
        )
        ...
        use_RFC = False
        print(
            f"WARNING: The routing period is greater than one hour. The RFC DA for {lake_number} \
