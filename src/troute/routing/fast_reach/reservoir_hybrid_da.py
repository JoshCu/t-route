...
def _modify_for_projected_storage(
    ...
    if projected_storage < min_storage and projected_storage > 0:
        ...
        LOG.warning(
            "WARNING: Modified release to maintain minimum storage for reservoir %s", lake_number
        )
        LOG.warning("at %s seconds after model start time.", current_time)
    ...
    if projected_storage <= 0:
        outflow = inflow
        LOG.warning(
            "WARNING: Modified release to prevent storage deficit for reservoir %s", lake_number
        )
        LOG.warning("at %s seconds after model start time.", current_time)
...
def reservoir_hybrid_da(
    ...
    LOG.debug(
        "Hybrid data assimilation for lake_id: %s at time %s from run start" % (lake_number, now)
    )
    ...
    if now >= update_time:
        ...
        if np.isnan(obs):  # no good observation was found
            ...
            """
            LOG.debug("No good observation found, persisting previously assimilated flow")
            ...
            if gage_lookback_seconds > obs_lookback_hours * 60 * 60:
                ...
                LOG.debug("observation at %s seconds from update time", gage_lookback_seconds)
                persisted_outflow = previous_persisted_outflow
                if now >= persistence_update_time:
                    new_persistence_index = persistence_index + 1
                    new_persistence_update_time = (
        ...
        new_persistence_index = persistence_index + 1
        new_persistence_update_time = persistence_update_time + persistence_update_time_interval
    ...
    if np.isnan(persisted_outflow):
        LOG.debug("Previously persisted outflow is nan, defaulting to levelpool outflow")
        # levelpool outflow
