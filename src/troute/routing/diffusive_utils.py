...
def fp_da_map(mx_jorder, ordered_reaches, usgs_df, nrch_g, t0, nsteps, dt_da_g, t0_g, tfin_g):
    """
        ...
        for i in range(len(timestamps)):
            if timestamps[i] not in usgs_df.columns:
                usgs_df_complete.insert(
                    i, timestamps[i], -4444.0 * np.ones(len(usgs_df)), allow_duplicates=False
                )
...
def fp_refactored_network_map(
    ...
    for i in range(len(head_rlink_trial)):
        ...
        trib_link = [
            k for k, v in refactored_diffusive_domain["incoming_tribs"].items() if v == rlnk
        ]
...
def fp_coastal_boundary_input_map(tw, coastal_boundary_depth_df, nrch_g, t0, t0_g, tfin_g):
    """
    Data assimilatoin data mapping between Python and Fortran
    Parameters
    ----------
...
def diffusive_input_data_v02(
    ...
    path_func = partial(
        nhd_network.split_at_waterbodies_and_junctions, set(junction_inflows.index.to_list()), rconn
    )
    ...
    for o, rch in jorder_reaches:
        ...
        if rch[0] not in rchhead_reaches:
            ...
            rchhead_reaches.update({rch[0]: {"number_segments": len(rch), "segments_list": rch}})
            # a list of segments for a given reach-bottom segment
            rchbottom_reaches.update({rch[-1]: {"number_segments": len(rch), "segments_list": rch}})
    ...
    for o in ordered_reaches.keys():
        for rch in ordered_reaches[o]:
            ...
            if i == 1:
                ...
                # elevation result
                dat_all[:, 2::3] = np.transpose(np.array(out_elv[:, 1 : len(rch_segs), j]))

            else:
                dat_all_c = np.empty((len(rch_segs) - 1, nts * 3))
                dat_all_c[:] = np.nan
                # flow result
                dat_all_c[:, ::3] = np.transpose(np.array(out_q[:, 1 : len(rch_segs), j]))
                # elevation result
                dat_all_c[:, 2::3] = np.transpose(np.array(out_elv[:, 1 : len(rch_segs), j]))
                # concatenate
