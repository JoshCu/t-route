...
def nwm_output_generator(
    ...
    if parity_parameters:
        ...
        parity_set["parity_check_file"] = parity_set.get("parity_check_file", parity_check_file)
    ...
    if (
        ...
        start = time.time()
        qvd_columns = pd.MultiIndex.from_product([range(nts), ["q", "v", "d"]]).to_flat_index()
        ...
        if return_courant:
            ...
            courant = pd.concat(
                [pd.DataFrame(r[2], index=r[0], columns=courant_columns) for r in results],
                copy=False,
            )
    ...
    if chrto:
        ...
        if chrtout_read_folder:
            chrtout_files = sorted(Path(chrtout_read_folder) / f for f in run["qlat_files"])
            ...
            if not logFileName == "NONE":
                with open(logFileName, "a") as preRunLog:
                    ...
                    preRunLog.write("-----\n")
                    preRunLog.write(
                        "Output of FVD files into folder: " + str(chrtout_read_folder) + "\n"
                    )
                    preRunLog.write("-----\n")
                preRunLog.close()
    ...
    if parquet_output_folder:
        ...
        # TO DO: create more descriptive filenames
        if supernetwork_parameters.get("title_string", None):
            filename_fvd = "flowveldepth_" + supernetwork_parameters["title_string"] + ".parquet"
            filename_courant = "courant_" + supernetwork_parameters["title_string"] + ".parquet"
        else:
            run_time_stamp = datetime.now().isoformat()
        ...
        timeseries_df = _parquet_output_format_converter(
            flowveldepth, restart_parameters.get("start_datetime"), dt, configuration, prefix_ids
        )
        ...
        ].to_parquet(output_path.joinpath(filename_fvd), allow_truncated_timestamps=True)

        if return_courant:
            courant = courant.sort_index()
            timeseries_courant = _parquet_output_format_converter(
                courant, restart_parameters.get("start_datetime"), dt, configuration, prefix_ids
            )
            timeseries_courant.loc[
                timeseries_courant["location_id"].isin(parquet_output_segments_str)
            ].to_parquet(output_path.joinpath(filename_courant), allow_truncated_timestamps=True)
        ...
        if nudging_true and lastobs_output_folder and not lastobs_df.empty:
            ...
            if not logFileName == "NONE":
                with open(logFileName, "a") as preRunLog:
                    ...
                    preRunLog.write("-----\n")
                    preRunLog.write(
                        "Output of lastobs data into folder: " + str(lastobs_output_folder) + "\n"
                    )
                    preRunLog.write("-----\n")
                preRunLog.close()
