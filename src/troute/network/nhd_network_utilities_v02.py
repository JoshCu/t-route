...
def organize_independent_networks(connections, wbody_break_segments, gage_break_segments):
    ...
    for tw, net in independent_networks.items():
        ...
        # break reaches at gages and junctions
            ...
            path_func = partial(nhd_network.split_at_gages_and_junctions, gage_break_segments, net)

        # break network at waterbody inlets/outlets and junctions
        elif wbody_break_segments and not gage_break_segments:
            path_func = partial(
                nhd_network.split_at_waterbodies_and_junctions, wbody_break_segments, net
            )
...
def build_forcing_sets(forcing_parameters, t0):
    ...
    if run_sets:
        ...
        for s, _ in enumerate(run_sets):
            ...
            final_timestamp_str = nhd_io.get_param_str(final_chrtout, "model_output_valid_time")
...
def build_qlateral_array(
    ...
    if qlat_input_folder:
        ...
        qlat_df = pd.DataFrame(np.stack(ql_list).T, index=idx, columns=range(len(qlat_files)))
    ...
    # if user-input timeslice folders are present, build TimeSlice sets
        ...
        for i, set_dict in enumerate(run_sets):
            ...
            if (usgs_timeslices_folder and nudging) or (usgs_timeslices_folder and usgs_da):
                filenames_usgs = (
                    timestamps.strftime("%Y-%m-%d_%H:%M:%S") + ".15min.usgsTimeSlice.ncdf"
                ).to_list()

                # identify available USGS TimeSlices
                filenames_usgs = _check_timeslice_exists(filenames_usgs, usgs_timeslices_folder)
            ...
            if usace_timeslices_folder and usace_da:
                filenames_usace = (
                    timestamps.strftime("%Y-%m-%d_%H:%M:%S") + ".15min.usaceTimeSlice.ncdf"
                ).to_list()

                # identify available USACE TimeSlices
                filenames_usace = _check_timeslice_exists(filenames_usace, usace_timeslices_folder)
...
def build_data_assimilation_lastobs(data_assimilation_parameters):
    ...
    if streamflow_da_parameters:
        ...
        if nudging:
            ...
            lastobs_type = streamflow_da_parameters.get("wrf_lastobs_type", "error-based")
