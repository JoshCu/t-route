...
class AbstractDA(ABC):
    ...
    __slots__ = [
        ...
        "_nStations_usgs",
        "_usgs_Array_datesSecondsArray_reservoir_usgs",
            ...
            if not from_files:
                if legacy_bmi_df:
                    ...
                    if len(usgs_Array) > 0:
                        ...
                        df_raw_usgs = a2df._unflatten_array(usgs_Array, nDates_usgs, nStations_usgs)

                        # Decode time/date axis
                        timeAxisName = "time"
                        freqString = "5T"
                        df_withDates_usgs = a2df._time_retrieve_from_arrays(
                            df_raw_usgs, dateNull, datesSecondsArray_usgs, timeAxisName, freqString
                        )
...
class PersistenceDA(AbstractDA):
    ...
    def __init__(self, network, from_files, value_dict, da_run=[]):
        ...
        reservoir_da_parameters = data_assimilation_parameters.get("reservoir_da", {}).get(
        ...
        if not from_files:
                ...
                if legacy_bmi_df:
                    ...
                    if len(usgs_reservoir_Array) > 0:
                        ...
                        df_raw_reservoirUsgs = a2df._unflatten_array(
                            usgs_reservoir_Array, nDates_reservoir_usgs, nStations_reservoir_usgs
                        )
                ...
                if legacy_bmi_df:
                    ...
                    if len(usace_reservoir_Array) > 0:
                        ...
                        df_raw_reservoirUsace = a2df._unflatten_array(
                            usace_reservoir_Array, nDates_reservoir_usace, nStations_reservoir_usace
                        )
            ...
            if usgs_persistence:
                ...
                if not self._usgs_df.empty:
                    ...
                    gage_link_df = network.link_gage_df["gages"].reset_index().set_index(["gages"])
                    ...
                    usgs_df_15min = self._usgs_df.transpose().resample("15min").asfreq().transpose()
            ...
            if 2 in network.waterbody_types_dataframe["reservoir_type"].unique():
                if self.reservoir_usgs_df.empty and len(self.reservoir_usgs_param_df.index) > 0:
                    self._reservoir_usgs_df = pd.DataFrame(
                        data=np.nan, index=self.reservoir_usgs_param_df.index, columns=[network.t0]
                    )
            ...
            if 3 in network.waterbody_types_dataframe["reservoir_type"].unique():
                if self.reservoir_usace_df.empty and len(self.reservoir_usace_param_df.index) > 0:
                    self._reservoir_usace_df = pd.DataFrame(
                        data=np.nan, index=self.reservoir_usace_param_df.index, columns=[network.t0]
                    )
...
class great_lake(AbstractDA):
    ...
    def __init__(self, network, from_files, value_dict, da_run):
        ...
        data_assimilation_parameters = self._data_assimilation_parameters
        run_parameters = self._run_parameters
        reservoir_persistence_da = data_assimilation_parameters.get("reservoir_da", {}).get(
        ...
        if greatLake:
            GL_crosswalk_df = pd.DataFrame(
                {"link": [4800002, 4800004, 4800006], "gages": ["04127885", "04159130", "02HA013"]}
            ).set_index("link")
        ...
        LOG.debug(
            "great_lake class is completed in %s seconds." % (time.time() - great_lake_start_time)
        )
...
def _create_usgs_df(
    data_assimilation_parameters, streamflow_da_parameters, run_parameters, network, da_run
):
    """
    ...
    usgs_files = [usgs_timeslices_folder.joinpath(f) for f in da_run["usgs_timeslice_files"]]
...
def _create_reservoir_df(
    ...
    res_files = [res_timeslices_folder.joinpath(f) for f in da_run[res_source + "_timeslice_files"]]
    ...
    if reservoir_df.empty == False:
        reservoir_param_df = pd.DataFrame(data=0, index=reservoir_df.index, columns=["update_time"])
...
def build_lastobs_df(
    ...
    LOG.debug(
        "Building last observation dataframe completed in %s seconds."
...
def read_reservoir_parameter_file(
    ...
    LOG.debug(
        "Reading and processing reservoir is completed in %s seconds."
...
def _read_timeseries_files(filepath, timeseries_dates, t0, final_persist_datetime):
    ...
    df = pd.DataFrame(
        [f.split("/")[-1].split(".") for f in files], columns=["Datetime", "dt", "ID", "rfc", "ext"]
    )
    ...
    for f in file_list:
        ...
        df = (
            ...
            .sort_values("forecastInd")[
                ["stationId", "discharges", "synthetic_values", "totalCounts", "timeSteps"]
            ]
        )
...
def assemble_rfc_dataframes(rfc_timeseries_df, rfc_lake_gage_crosswalk, t0, rfc_parameters):
    ...
    reservoir_rfc_param_df = (
        rfc_df[["stationId", "totalCounts", "timeseries_idx", "file", "use_rfc", "da_timestep"]]
        .drop_duplicates()
        .set_index("stationId")
    )
    reservoir_rfc_param_df = (
...
def _read_lastobs_file(
    ...
    LOG.debug(
        "Reading last observation file is completed in %s seconds."
...
def _create_GL_dfs(GL_crosswalk_df, data_assimilation_parameters, run_parameters, da_run, t0):
    ...
    if usgs_files:
        ...
        usgs_GL_df = (
            pd.melt(usgs_GL_df, var_name="Datetime", value_name="Discharge", ignore_index=False)
            .dropna()
            .reset_index()
        )
    ...
    if canadian_files:
        ...
        canadian_GL_df = (
            pd.melt(canadian_GL_df, var_name="Datetime", value_name="Discharge", ignore_index=False)
            .dropna()
            .reset_index()
        )
