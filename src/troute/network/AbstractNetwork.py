...
from troute.network.nhd_network_utilities_v02 import organize_independent_networks
...
class AbstractNetwork(ABC):
    ...
    def assemble_forcings(
        ...
        LOG.debug(
            "lateral inflow DataFrame creation complete in %s seconds." % (time.time() - start_time)
        )
    ...
    def new_q0(self, run_results):
        ...
        self._q0 = pd.concat(
            [
                pd.DataFrame(r[1][:, [-3, -3, -1]], index=r[0], columns=["qu0", "qd0", "h0"])
    ...
    def initial_warmstate_preprocess(self, from_files, value_dict):
        ...
        if self.break_points["break_network_at_waterbodies"]:
                ...
                # read waterbody initial states from WRF-Hydro type restart file
                    ...
                    waterbodies_initial_states_df = nhd_io.get_reservoir_restart_from_wrf_hydro(
                        ...
                        restart_parameters.get(
                            "wrf_hydro_waterbody_ID_crosswalk_file_field_name", index_id
                        ),
            ...
            LOG.debug(
                "waterbody initial states complete in %s seconds." % (time.time() - start_time)
            )
            start_time = time.time()
    ...
    def build_forcing_sets(
        ...
        if forcing_glob_filter == "nex-*":
            ...
            run_sets = [{"qlat_files": all_files, "nts": nts, "final_timestamp": final_timestamp}]
        ...
        # TODO: Throw errors if insufficient input data are available
            ...
            forcing_filename_list = [d_str + forcing_glob_filter[1:] for d_str in datetime_list_str]
            ...
            for f in forcing_filename_list:
                try:
                    ...
                    raise AssertionError(
                        "Aborting simulation because forcing file", J, "cannot be not found."
                    ) from None
    ...
    def filter_diffusive_nexus_pts(
        ...
        nexus_latlon["id"] = (
            nexus_latlon["id"].str.split("-", expand=True).loc[:, 1].astype(float).astype(int)
        )
...
def rewrite_to_parquet(file_args, binary_folder):
    ...
    df = pd.read_csv(
        f"{binary_folder}/tempfile_{tempfile_id}.csv", names=["feature_id", output_file_id]
    )
...
def read_DFlow_output(ds):
    ...
    df["station_name"] = (
        df["station_name"].str.decode("utf-8").str.extract(r"(\d+)").astype(float).astype(int)
    )
    df = (
