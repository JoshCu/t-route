...
from troute.network.nhd_network import reverse_network, reachable
from troute.network.nhd_network_utilities_v02 import (
    ...
    def update_routing_domain(self, dataframe, connections, waterbody_dataframe):
        ...
        for tw in diffusive_domain:
            ...
            if 999999 in headlink_mainstem:
                ...
                twlink_mainstem = tw
                diffusive_domain_all[twlink_mainstem] = self.diffusive_domain_by_both_ends_streamid(
                    connections, headlink_mainstem, twlink_mainstem, rfc_val, rpu_val
                )
...
class MCwithDiffusiveNatlXSectionRefactored(MCwithDiffusive):
    ...
    def refactored_reaches(self):
        if not self._refactored_reaches:
            ...
            for tw in self._diffusive_domain:
                ...
                refactored_diffusive_network_data[refac_tw]["upstream_boundary_link"] = (
                    self._diffusive_network_data[tw]["upstream_boundary_link"]
                )
        return self._refactored_reaches
    ...
    def unrefactored_topobathy_df(self):
        if self._unrefactored_topobathy_df.empty:
            ...
            self._unrefactored_topobathy_df.index = self._unrefactored_topobathy_df.index.astype(
                int
            )
        return self._unrefactored_topobathy_df
